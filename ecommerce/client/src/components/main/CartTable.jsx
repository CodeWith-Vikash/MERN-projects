...
const CartTable = ({ cart,setcart }) => {
  ...
  const handleDelete=(item)=>{
    ...
    if(userdata){
      setdeleting(true)
      axios.patch(`${baseurl}/api/cart/remove/${userdata._id}`,{productId:product._id}).then((result)=>{
        console.log(result)
        setcart(result.data.cart.items)
      }).catch((err)=>{
        console.log(err)
      ...
      toast.error('no userdata avilable try again')
    }
  }
    ...
    () => [
      ...
      },
      {
        ...
        Cell: ({ row }) => {
          ...
          const actualprice= (product?.price-((product?.price*product?.discount)/100)).toFixed(0)
      ...
      },
      {
        ...
        Cell: ({ row }) => (
          ...
          <button className="bg-red-500 text-white py-1 px-3 rounded hover:bg-red-600 transition text-xs md:text-sm flex items-center justify-center gap-1" onClick={() => handleDelete(row.original)}>
          Delete
            ...
            <tr {...headerGroup.getHeaderGroupProps()} className="bg-gray-200">
              {headerGroup.headers.map((column) => (
                <th
                  ...
                  className="px-2 py-3 text-left text-gray-600 font-medium text-xs md:text-sm lg:text-base"
                >
            ...
            return (
              <tr {...row.getRowProps()} className="hover:bg-gray-50">
                {row.cells.map((cell) => (
                  <td
                    {...cell.getCellProps()}
                    className="px-2 py-4 text-gray-900 text-xs md:text-sm lg:text-base"
                  >
                    {cell.render('Cell')}
                  </td>
                ))}
