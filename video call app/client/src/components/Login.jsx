  ...
  return (
    <div className="h-screen bg-zinc-800 flex justify-center items-center">
      ...
      <form className="flex flex-col gap-3" onSubmit={handleLogin}>
        ...
        {isopen? <RxEyeOpen  className="absolute top-3 right-3 cursor-pointer text-orange-500"size="1.3rem" onClick={toggleye}/> : <RiEyeCloseFill className="absolute top-3 right-3 cursor-pointer text-orange-500"size="1.3rem" onClick={toggleye}/>}
        </div>
        <button className="bg-orange-500 text-white font-semibold py-1 px-2 rounded">{loading?'Loging...':'Login'}</button>
        <p>you don't have an account ? <NavLink to="/signup">
        <b className='text-orange-500'>signup</b>
          </NavLink></p>
      </form>
      </div>
    </div>
