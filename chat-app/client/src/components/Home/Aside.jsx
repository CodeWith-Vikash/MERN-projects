    ...
    <div className='flex flex-col gap-4'>
        <nav className='h-[9.2vh] bg-black flex items-center justify-between px-2 w-full'>
          <div className='flex items-center gap-1'>
            <img src={currentUser?.photoURL} alt="" className='h-10 rounded-full w-10 object-cover'/>
            <span>{currentUser?.displayName}</span>
          </div>
           <div className='flex items-center gap-2'>
           <button className='bg-white text-black font-semibold py-1 px-2 rounded text-sm' onClick={()=> navigate('/login')}>Logout</button>
