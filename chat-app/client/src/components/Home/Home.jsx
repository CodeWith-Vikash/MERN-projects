...
const Home = () => {
  const {userdata,openSide,closeSide,sideref,mainloading}=useContext(UserContext)
  return (
    <>
      {!userdata?
      <section className='bg-gray-700 h-screen w-full flex flex-col items-center pt-10 gap-2'>
        ...
        {mainloading?<button className='bg-yellow-400 text-white py-1 px-4 text-lg font-semibold rounded-lg flex gap-1 items-center'>
          <span>Wait</span>
          <img src="\loader.gif" className='h-4 rounded-full'/>
