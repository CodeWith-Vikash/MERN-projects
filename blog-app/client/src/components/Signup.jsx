    ...
    <div className={`h-screen flex items-center justify-center ${isdark?'bg-gray-900 text-white':'bg-white text-black'}`}>
       <form action="/signup" method='post' onSubmit={handleSubmit} className='flex flex-col gap-4 bg-violet-800 p-4 rounded-lg'>
         ...
         <input type="text" placeholder='Password' name='Password' ref={passwordref}
           className='outline-none border-none px-4 py-1 text-lg rounded text-black'
         />
          <label htmlFor="file" className='flex items-center font-semibold gap-4 cursor-pointer'>
            <p>add an Avtar</p>
            <FaRegImage size='1.5rem'/>
            {imageSrc && <img src={imageSrc} alt="Selected" className="w-[50px] h-[50px] object-cover rounded shadow-lg" />}
          </label>
          <input type="file" onChange={handleFileChange} id="file" className='hidden'/>
         <button type='submit' className='bg-green-600 px-2 py-1 rounded font-semibold'>Sign up</button>
          <p>you already have an account? <Link to='/login' className='text-black font-semibold'>login</Link></p>
       </form>
    </div>
  )
}
